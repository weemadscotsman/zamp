...
# References to autoloads (will be set in _ready)
...
func _ready():
	...
	# Get references to autoloads safely
	await get_tree().process_frame
	_audio_manager = get_node_or_null("/root/AudioManager")
	_global_state = get_node_or_null("/root/GlobalState")
	
	if _audio_manager == null:
		push_warning("DialogueManager: AudioManager autoload not found")
	if _global_state == null:
		push_warning("DialogueManager: GlobalState autoload not found")
...
func _type_text(text: String):
	...
	for i in range(text.length()):
		...
		if i % 3 == 0 and _audio_manager != null:
			_audio_manager.play_typing_sound()
...
func _apply_choice_effect(effect: Dictionary):
	...
	if _global_state == null:
		push_warning("GlobalState not available, can't apply choice effects")
		return
	
	# Apply stat changes, evidence discovery, etc.
	if effect.has("skepticism"):
		_global_state.skepticism += effect.skepticism
	if effect.has("guilt"):
		_global_state.guilt += effect.guilt
	if effect.has("evidence"):
		_global_state.add_evidence(effect.evidence)
	if effect.has("followers"):
		_global_state.followers += effect.followers
	if effect.has("reputation"):
		_global_state.reputation += effect.reputation
	
	_global_state.update_big_four()
	_global_state.save_game()
